...
Poll [`GET /payments_result`](/api/payments/payments-results) from your server using the generated `traceId`. The lifecycle can return `unpaid`, `confirming`, `paid_less`, `pending`, `auditing`, `success`, or `failed`; only `success` and `failed` are terminal. Keep the sale open for every non-terminal status. For `paid_less`, reuse the original order and trace when the customer pays the remaining amount. Fulfill only after your server verifies a `success` result. See [Payment Lifecycle](/api/payments/payment-lifecycle).
